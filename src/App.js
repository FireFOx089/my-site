...
import { loadPortfolioItems, loadFilters, DEFAULT_CREDITS } from './portfolioData';
...
// ─── PORTFOLIO PAGE ──────────────────────────────────────────
function PortfolioPage({ onClose, items: propItems, filters: propFilters }) {
  const items = propItems || loadPortfolioItems();
  const filters = propFilters || loadFilters();
  ...
  const filtered = activeFilter === 'All'
    ? shuffledAll
    : items.filter(p => p.cat === activeFilter);
  ...
  const filmstripImages = useMemo(() => {
    if (!selected) return [];
    if (selected.video) {
      const list = selected.images?.length ? selected.images : (selected.img ? [selected.img] : []);
      return Array.from(new Set(list));
    }
    return gallery ? gallery.slice(1) : [];
  }, [selected, gallery]);
          ...
          >
            ...
            >
              ...
              <div className="lb-images-col">
                ...
                {(filmstripImages.length > 0 || selected.subVideos?.length > 0) && (
                  <div className="lb-filmstrip">
                    {filmstripImages.map((src, i) => (
                      <LbFilmImage key={`img-${i}`} src={src} alt={`${selected.label} — ${i + 1}`} />
                    ))}
...
export default function App() {
  ...
  const [portfolioItems, setPortfolioItems] = useState(() => loadPortfolioItems());
  const [portfolioFilters, setPortfolioFilters] = useState(() => loadFilters());

  useEffect(() => {
    const handleLocationChange = () => {
      const path = (window.location.pathname || '').toLowerCase();
      const hash = (window.location.hash || '').toLowerCase();
      setIsAdmin(path === '/admin' || path.startsWith('/admin/') || hash === '#/admin' || hash === '#admin');
    };

    const handleDataUpdate = () => {
      setPortfolioItems(loadPortfolioItems());
      setPortfolioFilters(loadFilters());
    };
    ...
    return () => {
      ...
      window.removeEventListener('artsnfar_data_updated', handleDataUpdate);
